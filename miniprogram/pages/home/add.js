...
Page({
  ...
  data: {
    ...
    // 用于屏蔽对safeInfo增删的时候 影响formData其余字段
    protectData: {},
    adding: false
  },
  ...
  onLoad: function (options) {
    ...
    let self = this
    eventChannel.on('copyAccount', function(data) {
      self.setData({
        protectData: Object.assign({}, self.data.formData, data),
        formData: Object.assign({}, self.data.formData, data)
      })
    })
  },
  ...
  async addAccount({ detail }){
    try{
      ...
      const ret = await app.$api.account.create(params)
      this.setData({
        adding: false
      })
      app.$toast('创建成功， 即将返回首页')
      setTimeout(() => {
        wx.navigateBack({})
      }, 1000)
    }catch(error){
